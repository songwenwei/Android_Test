...
//股票/港股
public class F5_Stock_StockOfHK extends StockActivity {

	public void testCase001() throws Exception {
		CaseName = "港股_进入分时页面";
		new F5StockActivity().click_Stockname("0001.HK");

		String F5_title = ((TextView) this.getViewById("book_name")).getText()
				.toString();// 获取分时页面标题

		CaseCheck = this.EqualsChecked(F5_title.contains("长江实业"));// 校验分时页面标题
	...
	public void testCase013() throws Exception {
		...
		CaseCheck = this.EqualsChecked(solo.searchText("长江实业"));
		System.out.println(CaseCheck);
	}

	public void testCase014() throws Exception {
		CaseName = "港股_横屏点击【5分K】";
		new F5StockActivity().click_Stockname("0001.HK");
	...
	public void testCase019() throws Exception {
		...
		CaseCheck = this.EqualsChecked(time1.equalsIgnoreCase("09:30")
				&& time2.equalsIgnoreCase("12:00/13:00")
				&& time3.equalsIgnoreCase("16:00"));// 检验時間
		System.out.println(CaseCheck);

	}

	public void testCase020() throws Exception {
		CaseName = "港股_竖屏页面选择分时横屏";
		new F5StockActivity().click_Stockname("0001.HK");
