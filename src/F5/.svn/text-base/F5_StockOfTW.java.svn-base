...
//通用/通用即时走势/台股
public class F5_StockOfTW extends StockActivity {
	...
	public void testCase001() throws Exception {
		...
		float x = location[0] + (viewWidth / 4 * 1.0f);
		float y = location[1];
		...
		String time2 = ((TextView) F5_time.getChildAt(10)).getText().toString();// 获取分时走势收盘时间
			...
			CaseCheck = this.EqualsChecked(time1.equalsIgnoreCase("09:00") && time2.equalsIgnoreCase("13:30"));// 检验時間
	...
	public void testCase008() throws Exception {
		...
		CaseCheck = this.EqualsChecked(solo.searchText("彩晶"));
		System.out.println(CaseCheck);
	}
